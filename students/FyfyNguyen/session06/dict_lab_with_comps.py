...
# divisible 2, 3, and 4
s2 = {i in range(21) if not i % 2}
s3 = {i in range(21) if not i % 3}
s4 = {i in range(21) if not i % 4}
...
n = 1
sequence = range(2, n + 1)
my_sets = [set() for b in sequence]

for a, b in zip(sequence, my_sets):
    b.add(c) for c in range(21) if not c % i

new_set = [{i for i in range(21) if not i % n} for j in range(2, n + 1)]
